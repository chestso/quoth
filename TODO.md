...
- [x] Chat commands live under the free `C-c c` prefix (`quoth-chat-command-map`): `s` send, `i` interrupt, `k` clear, `r` toggle reasoning, `m` select model, `a` attach image, `t` toggle image attach
...
- [ ] Keybindings for common operations beyond `C-c c` (permission handling for a future `ask`/`allowlist`; interrupt is already `C-c c i`, model selection `C-c c m`)
