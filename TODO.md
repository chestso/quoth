...
- [ ] Model catalog from `GET /v1/models` (public, no auth): confirm which Hyper endpoint serves the OpenAI-compatible public list; currently `quoth-select-model` falls back to the static list on a cold cache and the refresh fetches the authenticated `GET /v1/provider` catalog. Add reasoning-effort selection by model
