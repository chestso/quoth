...
- [ ] Model catalog source: confirmed `GET /v1/models` is live and public (OpenAI-shaped `{"object": "list", "data": [...]}`, same id set as `/v1/provider`, which is also unauthenticated — no auth on either). Quoth keeps `/v1/provider` as its wired source because `quoth-hyper--normalize-model` already speaks its schema; switching to `/v1/models` would need a second normalizer for its different shape (`reasoning.effort_levels` as `{value, display}` objects, `capabilities.vision`, `pricing.*`). Reasoning-effort selection per model already works off catalog levels (`e` in C-c c m); the cold-cache fallback offers only the single static default (no effort levels) until the background refresh lands, and the buffer-init prefetch narrows that window.
