...
- **hyper** fetches the OpenAI-compatible `GET /v1/models` (one request) and
  normalizes the payload through `quoth-hyper--catalog-parse` +
  `quoth-hyper--normalize-model`.
...
`quoth-provider--models-seed` generic's plists stamped `FETCHED-AT` 0 (stale),
so the read returns immediately through the stale-while-revalidate path and the
kicked refresh overrides the seed with live data. The hyper seed reads the
tracked `quoth-hyper-models.json` snapshot (the verbatim `/v1/models` payload,
...
- **The catalog is one request.** `GET /v1/models` returns the whole catalog in
  the OpenAI list shape; it is normalized through `quoth-hyper--catalog-parse` +
  `quoth-hyper--normalize-model` (see the catalog cache above), and the bundled
  seed is the verbatim `/v1/models` payload.
...
Thinking is bi-state — unset (omit the key) or off (send `thinking: false`) —
with no provider-specific code: `quoth-openai-compose-request` emits
`thinking: false` from the session slot when set and omits the key otherwise;
there is no `thinking: true` path (a validated no-op on the hyper gateway, where
every thinking model streams reasoning by default). The server accepts unknown
params silently (HTTP 200); ollama currently ignores the key, so "off" is a
no-op rather than a suppression there.
