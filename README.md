...
Both carry version tags (`v0.2.1`). A plain clone gets the latest commit, or pin to a tag for a stable release (see below).
...
(package-vc-install
 '(quoth :url "https://github.com/chestso/quoth.git"
         :branch "v0.2.1"))         ; omit :branch for latest
```
...
```sh
git clone --branch v0.2.1 https://github.com/chestso/quoth.git
```
