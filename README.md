...
Both carry version tags (`v0.11.0`). A plain clone gets the latest commit, or
...
(use-package quoth
  :vc (:url "https://github.com/chestso/quoth.git"
        :branch "v0.11.0")            ; omit :branch for latest
...
```sh
git clone --branch v0.11.0 https://github.com/chestso/quoth.git
```
...
- `C-c " m` — open the model selector: pick a model, switch the active provider
  (`p`), toggle reasoning off (models reason by default), set a reasoning-effort
  level, or use `d` to reset the per-session attributes to the provider defaults
...
catalog, switch the buffer to another provider (`p`), toggle reasoning off
(every thinking model reasons by default; the toggle sends `thinking: false`),
set a reasoning-effort level, or use `d` to reset the per-session attributes to
the provider defaults. The model read annotates every candidate with its display
name, context window, per-1M-token prices (input, output, cache-write,
cache-hit), reasoning-effort levels, and vision support — each part shown only
when the catalog reports it, and rendered by every completion UI (the stock
`*Completions*` buffer, icomplete, vertico) through the standard completion
affixation. The menu's model line carries the same annotation text for the
selected model. The catalog is seeded from a bundled snapshot of the provider's
model list (regenerated by `make models`), so prices and context windows show
for each model even before the first fetch, and a live refresh (`g` in the
selector) keeps it current.
