...
Both carry version tags (`v0.7.0`). A plain clone gets the latest commit, or pin to a tag for a stable release (see below).
...
(use-package quoth
  :vc (:url "https://github.com/chestso/quoth.git"
        :branch "v0.7.0")            ; omit :branch for latest
  :bind ("C-c c" . quoth)
...
```sh
git clone --branch v0.7.0 https://github.com/chestso/quoth.git
```
